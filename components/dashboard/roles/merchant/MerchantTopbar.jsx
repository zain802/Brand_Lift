...
import { Menu as MenuIcon, LogOut, ChevronDown, UserCircle, Bell } from "lucide-react";
...
const MerchantTopbar = ({ onMenuClick }) => {
    ...
    return (
        <header className="h-16 bg-white border-b border-gray-100 px-4 md:px-8 flex items-center justify-between gap-4 sticky top-0 z-40">
            <div className="flex items-center gap-3 min-w-0">
                ...
                <h1 className="text-base md:text-xl lg:text-2xl font-black text-gray-800 truncate uppercase tracking-tighter">
                    Branding <span className="text-purple-600">Ups</span>
                    <span className="hidden sm:inline-block mx-3 text-gray-200">|</span>
                    <span className="text-sm font-black text-gray-400 tracking-[0.2em]">{userRole.replace('_', ' ')}</span>
                </h1>
            </div>
                ...
                >
                    <div className="flex items-center gap-3 cursor-pointer p-1 rounded-full hover:bg-gray-50 transition-colors">
                        <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-white shadow-md shrink-0 transition-transform group-hover:scale-105 bg-gray-100">
                            {/* Fix for Image error - using standard img with fallback or Image with unoptimized */}
                            <img
                                src="https://randomuser.me/api/portraits/men/32.jpg"
                                alt="Profile"
                                className="w-full h-full object-cover"
                                onError={(e) => {
                                    e.target.src = "https://ui-avatars.com/api/?name=" + userName + "&background=7c3aed&color=fff";
                                }}
                            />
                        </div>
                    ...
                    {isDropdownOpen && (
                        <div className="absolute top-full right-0 mt-1 w-56 bg-white rounded-2xl shadow-2xl border border-gray-50 py-2 animate-in fade-in zoom-in-95 duration-200 z-50">
                            ...
                            <button className="w-full flex items-center gap-3 px-5 py-3 text-sm font-bold text-gray-600 hover:bg-purple-50 hover:text-[#7c3aed] transition-colors">
                                <UserCircle size={18} />
                                Profile Details
                            </button>
                            <button
